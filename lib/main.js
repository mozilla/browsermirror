...
function makeId() {
  if (simplePrefs.prefs.groupId) {
    return simplePrefs.prefs.groupId;
  }
  var id = btoa(Math.random()).replace(/=/g, '').substr(0, 10);
  // FIXME: not sure if this is sticky
  simplePrefs.prefs.groupId = id;
  return id;
}
