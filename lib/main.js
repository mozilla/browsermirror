...
Sharer.prototype = {
  ...
  attachWorker: function () {
    ...
    this.socketProxier = new SocketProxier(this.worker);
...
function SocketProxier(consumerWorker) {
  this.bindings = [];
  this.consumerWorker = consumerWorker;
  this.wsWorker = Page({
    contentScriptFile: data.url("wsecho.js"),
    contentURL: getServer() + "/blank.html"
  });
  this.echo(["eventOpened", "eventClosed", "dataReceived"], this.wsWorker, this.consumerWorker);
  this.echo(["openConnection", "close", "sendData"], this.consumerWorker, this.wsWorker);
...
SocketProxier.prototype = {
  ...
  echo: function (names, fromWorker, toWorker) {
    ...
    names.forEach(function (name) {
      ...
      self.bindings.push([fromWorker, name, func]);
    });
  }
};
