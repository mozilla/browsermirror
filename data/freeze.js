...
Freeze.serializeElement = function (el, includeHTML) {
  ...
  if (el.tagName == 'IFRAME') {
    ...
    var html;
    try {
      html = this.staticHTML(el.contentWindow.document.documentElement);
    } catch (e) {
      ...
      for (i=curPos; i<curNext; i++) {
        if (typeof curChildren[i] == "string") {
          ...
          if (! curChildren[i].jsmirrorId) {
            ...
            Freeze._elements[curChildren[i].jsmirrorId] = curChildren[i];
          }
...
Freeze.applyDiff = function (commands) {
  var pushes, j, child;
...
Freeze.refreshElements = function () {
  ...
  recur(Freeze._elements, document.head);
  recur(Freeze._elements, document.body);
};
